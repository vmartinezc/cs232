...
@modified by: Valeria Martinez Castro
@date 02/10/2020
...
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, num=0, batchMode=False):
        ...
        self._os = os
        self._debug = debug
        self._batchMode = batchMode  #make a CPU object called batchedMode that will allow the running of the batch

        #if it is in batchmode 
        if batchMode:
            self._batchIterator = startAddr     #set the batch iterator to the initial address
            self._registers["pc"]=self._ram[self._batchIterator]   #set the pc register to be the index of the first program in the RAM
        else:   
            self._batchIterator = 0
    ...
    def _run_program(self):
        while True:
            ...
            if not self.parse_instruction(self._ram[self._registers['pc']]):
                ...
                if self._batchMode & self._batchIterator+1 >0:
                    self._registers ={
                        'reg0' : 0,
                        'reg1' : 0,
                        'reg2' : 0,
                        'pc': self._ram[self._batchIterator+1]

                    }
                    self._batchIterator += 1
                    continue
